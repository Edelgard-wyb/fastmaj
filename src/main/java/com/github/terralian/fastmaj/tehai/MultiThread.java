...
public  class MultiThread<H,T>{
     ...
     */
    public List<T> outExecute(int currentThread, List<H> data) {
        List<Pair>result=new ArrayList<>();
        H h = data.get(0);
        if(h instanceof Cards){
            Cards cards =(Cards)h;
            Pair<IHai, List<IHai>> iHaiListPair = cards.onExecute();
            if(iHaiListPair.getValue().size()>0){
                result.add(iHaiListPair);
            }
        }
